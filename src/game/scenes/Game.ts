...
export class Game extends Scene {
    ...
    createPlatforms() {
        ...
        for (let i = 0; i < this.numberOfPlatforms; i++) {
            ...
            //Agregar Moneda
            const coin = this.createCoin(x)
            this.physics.add.collider(this.player,  platform);
            this.physics.add.collider(coin, platform);
        }
    }

    createCoin(x: number): Phaser.Types.Physics.Arcade.ImageWithDynamicBody {
        //Monedas
        const coin = this.physics.add.image(x, this.player.y - 50, "coin"); // Usar la posición Y del jugador
        coin.setScale(0.05);
    ...
    update(time: number, delta: number): void {
        ...
        this.children.each((coin: any) => {
            ...
            if (coin.getData("isCoin") && this.physics.overlap(this.player, coin)) {
                alert("Aqui pregunta ?")
